...
from cartpole.a2c.models import Actor, Critic
...
class Agent:
    ...
    def __init__(self, env: ENV, gamma: float = 0.99) -> None:
        """soon"""
        self.env: ENV = env
        self.actor: Actor = Actor(alpha=0.01, num_actions=2, num_hidden_units=128)
        self.critic: Critic = Critic(alpha=0.01, num_hidden_units=128)
        self.max_steps: int = 1000
        self.gamma: float = gamma
        self.reward_history: list[float] = []
        self.loss_history: list[float] = []

    def _tf_env_reset(self) -> tf.Tensor:
        """soon"""
        return tf.py_function(self.env.reset, [], tf.float32)
    ...
    def training(
        self,
        rewards_threshold: int = 195,
        min_episodes_criterion: int = 500,
